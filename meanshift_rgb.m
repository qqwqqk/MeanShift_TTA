...
%读取图像并进行目标选定
image_cache = imread(imgfile(1).name);
crop_size = [170,320,119,79];       %目标定位窗
[img_crop]=imcrop(image_cache, crop_size);
...
for i=1:img_height
    for j=1:img_width   
        img_r = fix(double(img_crop(i,j,1))/16);
        img_g = fix(double(img_crop(i,j,2))/16);
        img_b = fix(double(img_crop(i,j,3))/16);
        index = img_r*256 + img_g*16 + img_b;
...
for img_index=2:lengthfile  
    img_cache = imread(imgfile(img_index).name);  
    iterator = 0;  
    center_move=[2,2];  
    ...
    while((center_move(1)^2 + center_move(2)^2>0.5) && iterator<20)   %迭代条件  
        ...
        for i=1:img_height
            for j=1:img_width   
                img_r = fix(double(iter_cache(i,j,1))/16);
                img_g = fix(double(iter_cache(i,j,2))/16);
                img_b = fix(double(iter_cache(i,j,3))/16);
                index = img_r*256 + img_g*16 + img_b;
